...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # otherwise False.
        ############
        veh_pos = np.ones((4, 1))  # homogeneous coordinates
        veh_pos[0:3] = x[0:3]
        sens_pos = self.veh_to_sens * veh_pos  # transform from vehicle to lidar coordinates

        # compute angle from x value
        angle = math.atan2(sens_pos[1], sens_pos[0])
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            veh_pos = np.ones((4, 1))  # homogeneous coordinates
            veh_pos[0:3] = x[0:3]
            sens_pos = self.veh_to_sens * veh_pos  # transform from vehicle to camera coordinates
            ...
            # make sure to not divide by zero, raise an error if needed
            try:
                h_x[0, 0] = self.c_i - self.f_i * sens_pos[1] / sens_pos[0]
                h_x[1, 0] = self.c_j - self.f_j * sens_pos[2] / sens_pos[0]
